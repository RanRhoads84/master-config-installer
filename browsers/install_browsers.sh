...
show_menu() {
    ...
    echo -e "${CYAN}3. ${NC}Firefox ESR"
    echo -e "${CYAN}4. ${NC}LibreWolf"
    echo -e "${CYAN}5. ${NC}Brave"
    echo -e "${CYAN}6. ${NC}Floorp"
...
install_helium() {
    ...
    sudo rm -f /etc/apt/sources.list.d/helium-deb-repo.list
    sudo rm -f /usr/share/keyrings/helium-deb-repo.gpg

    # Add Helium Browser GPG key
    echo -e "${CYAN}Adding Helium Browser repository key...${NC}"
    if ! curl -fsSL https://justaguylinux.codeberg.page/helium-deb-repo/key.asc | sudo gpg --dearmor -o /usr/share/keyrings/helium-deb-repo.gpg; then
        echo -e "${RED}Failed to add Helium Browser repository key.${NC}"
        return 1
    fi

    # Add Helium Browser repository
    echo -e "${CYAN}Adding Helium Browser repository...${NC}"
    echo "deb [arch=amd64 signed-by=/usr/share/keyrings/helium-deb-repo.gpg] https://justaguylinux.codeberg.page/helium-deb-repo stable main" | sudo tee /etc/apt/sources.list.d/helium-deb-repo.list > /dev/null

    # Update and install
    echo -e "${CYAN}Installing Helium Browser package...${NC}"
    sudo apt update
